...
class TestBoard:
    ...
    def test_no_violations(self):
        ...
        n = Location('N', '', 0, 50, 'country', 'iso2',
                     'iso3', 0, 0, Continent.EU)
        board.put(n, *board.coords(0, -4, centered=True))
        assert not board.violations()
        s = Location('S', '', 0, -50, 'country',
                     'iso2', 'iso3', 0, 0, Continent.AF)
        board.put(s, *board.coords(0, 4, centered=True))
        assert not board.violations()
        e = Location('E', '', 50, 0, 'country', 'iso2',
                     'iso3', 0, 0, Continent.OC)
        board.put(e, *board.coords(4, 0, centered=True))
        assert not board.violations()
        w = Location('W', '', -50, 0, 'country',
                     'iso2', 'iso3', 0, 0, Continent.AS)
        board.put(w, *board.coords(-4, 0, centered=True))
        assert not board.violations()
        n_e = Location('NE', '', 50, 50, 'country',
                       'iso2', 'iso3', 0, 0, Continent.NA)
        board.put(n_e, *board.coords(4, -4, centered=True))
        assert not board.violations()
        n_w = Location('NW', '', -50, 50, 'country',
                       'iso2', 'iso3', 0, 0, Continent.SA)
        board.put(n_w, *board.coords(-4, -4, centered=True))
        assert not board.violations()
        s_e = Location('SE', '', 50, -50, 'country',
                       'iso2', 'iso3', 0, 0, Continent.OC)
    ...
    def test_violations(self):
        ...
        # left of center
        loc = Location('L', '', 50, 0, 'country', 'iso2',
                       'iso3', 0, 0, Continent.AN)
        board.put(loc, *board.coords(-2, 0, centered=True))
        vs = board.violations()
        assert len(vs) == 1 and vs[0].direction == Direction.LONGITUDE
        board.remove(*board.coords(-2, 0, centered=True))
        # above of center
        loc = Location('L', '', 0, -50, 'country',
                       'iso2', 'iso3', 0, 0, Continent.AN)
        ...
        # below of center
        loc = Location('L', '', 0, 50, 'country', 'iso2',
                       'iso3', 0, 0, Continent.AF)
        old_placed = board.placed()
